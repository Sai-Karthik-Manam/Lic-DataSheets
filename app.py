...
def init_db():
    ...
    try:
        ...
        # Create indexes — each in its own try/except so one failure
        ...
        for idx_sql in indexes:
            try:
                ...
                app.logger.warning(f"Index creation warning (non-critical): {idx_err}")
    ...
    try:
        ...
        if USE_POSTGRESQL:
            ...
            if result:
                ...
                app.logger.info(f"OTP stored for {username}: valid for {int(remaining)} seconds")
        else:
            now = datetime.now()
            ...
            app.logger.info(f"OTP stored for {username}")
...
def verify_otp(username, otp_code):
    ...
    try:
        ...
        if USE_POSTGRESQL:
            ...
            if not result:
                app.logger.info(f"OTP not found or already used: username={username}")
                return False
...
def list_clients():
    ...
    try:
        ...
        return render_template('clients.html', clients=clients_list, sort_by=sort_by,
                               order=order_sql.lower(), search_query=search_query,
...
# ─── BUG FIX: upload() ────────────────────────────────────────────────────────
# Previously called gfile.SetContentFile(file) passing a Flask FileStorage object.
# PyDrive2 requires a file-system path (string).  File is now saved to a temp
# path first and cleaned up after the upload.  file.content_length was also
# unreliable (often None/0); replaced with os.path.getsize() on the temp file.
# ──────────────────────────────────────────────────────────────────────────────
    ...
    try:
        ...
        for doc_type in DOCUMENT_TYPES:
            ...
            if file and file.filename:
                ...
                # Save to temp path — PyDrive2 SetContentFile() needs a filepath string
                temp_path = os.path.join(UPLOAD_FOLDER, secure_filename(file.filename))
                try:
                    file.save(temp_path)
                    file_size = os.path.getsize(temp_path)  # reliable size from disk
                    ...
                    gfile.SetContentFile(temp_path)   # ← fixed: pass path, not object
                    gfile.Upload()
                    ...
                    if USE_POSTGRESQL:
                        cur.execute("""
                            ...
                            (client_id, doc_type, gfile['id'], gfile['title'],
                             file_url, file_size, file_mime, now, session.get('user_id')))
                    else:
                        cur.execute("""
                    ...
                    cleanup_temp_file(temp_path)   # always clean up temp file
...
def fetch_data():
    ...
    try:
        ...
        if USE_POSTGRESQL:
            cur.execute(
                "SELECT c.id, c.name, c.created_at, c.updated_at, c.folder_id FROM clients c WHERE c.name ILIKE %s",
                (f'%{client_name}%',))
        else:
            cur.execute(
                "SELECT c.id, c.name, c.created_at, c.updated_at, c.folder_id FROM clients c WHERE c.name LIKE ?",
                (f'%{client_name}%',))
...
# ─── BUG FIX: update_client_documents() ──────────────────────────────────────
# Same two bugs as upload(): SetContentFile() needs a filepath, not a file
# object; and file.content_length is unreliable.  Fixed with the same temp-file
# pattern.
# ──────────────────────────────────────────────────────────────────────────────
    ...
    try:
        ...
        for doc_type in DOCUMENT_TYPES:
            ...
            if file and file.filename:
                ...
                # Delete existing Drive file before uploading replacement
                try:
                    if USE_POSTGRESQL:
                        cur.execute(
                            "SELECT file_id FROM documents WHERE client_id = %s AND document_type = %s",
...
<th style="padding:10px; text-align:left;">IP</th></tr>'''
        ...
        for act in activities:
            html += (f'<tr style="border-bottom:1px solid #ddd;">'
                     f'<td style="padding:10px;">{act[0]}</td>'
                     f'<td style="padding:10px;">{act[1] or "-"}</td>'
                     f'<td style="padding:10px;">{act[2]}</td>'
                     f'<td style="padding:10px;">{act[3] or "-"}</td></tr>')
                        ...
                        if USE_POSTGRESQL:
                            cur.execute("""
                                ...
                                (client_id, doc_type, file_id, file['title'],
                                 file_url, file_size, mime_type, upload_time))
                            synced_count += 1
                        else:
                            try:
                                cur.execute("""
                                ...
                                cur.execute("""
                                    ...
                                    (file_id, file['title'], file_url,
                                     file_size, mime_type, upload_time, client_id, doc_type))
                    except Exception:
                        continue
...
def send_otp_email(email, otp_code, username):
    try:
        message = Mail(
            from_email=os.getenv('SMTP_USERNAME', 'lic.datasheets@gmail.com'),
            to_emails=email,
            subject='LIC Manager - Your Login OTP Code',
            html_content=f"""
                <div style="font-family:Arial,sans-serif;max-width:400px;margin:auto;padding:30px;border-radius:12px;border:1px solid #eee;">
                  <h2 style="color:#667eea;">LIC Manager - Login Verification</h2>
                  <p>Hello <strong>{username}</strong>,</p>
                  <p>Your OTP code is:</p>
                  <div style="font-size:36px;font-weight:bold;letter-spacing:8px;color:#667eea;padding:20px;background:#f0f2ff;border-radius:8px;text-align:center;">
                    {otp_code}
                  </div>
